...
{
    private readonly XmlSerializer _serializer = new(typeof(TContainer));
    ...
    private void AppendContainer(List<T> list, string file)
    {
        ...
        {
#if GTA
            Game.LogTrivial($"Error whilst attempting to read '{file}': {Environment.NewLine}{xe}");
#else
            AnsiConsole.MarkupLineInterpolated($"[red]Error whilst attempting to read '{file}'[/red]");
            AnsiConsole.WriteException(xe);
#endif
            return;
        }
